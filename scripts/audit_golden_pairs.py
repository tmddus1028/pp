...
def main():
    ...
    reports = []
    for number, application, count in [
        (1, "14/623,904", 19),
        (2, "15/914,356", 20),
        (3, "17/708,932", 20),
    ]:
        ...
        response = httpx.post(
            ...
            files={
                key: (p.name, b, "application/pdf")
                for key, p, b in zip(["patent", "office_action"], paths, data)
            },
            timeout=600,
        )
        result = response.json()
        (args.output / f"pair-{number}.json").write_text(
        ...
        if response.status_code == 200:
            ...
            check(
                "claims",
                [c["claim_number"] for c in result["patent"]["claims"]],
                list(range(1, count + 1)),
            )
            check(
                "global_direct",
                summary["direct_rejected_claims"],
                list(range(1, 20))
                if number == 1
                else list(range(1, 21))
                if number == 2
                else list(range(1, 15)) + [17, 18, 19],
            )
            check(
                "global_dependency",
                summary["dependency_impacted_claims"],
                [] if number < 3 else [15, 16],
            )
            ...
            check(
                "unique_relied_citations",
                len([c for c in result["citations"] if c["citation_role"] == "relied_upon"]),
                {1: 5, 2: 5, 3: 7}[number],
            )
            expected = {
            ...
            if number == 1:
                check(
                    "R1_dependency",
                    result["impacts"][0]["dependency_impacted_claims"],
                    [2, 5, 8, 11],
                )
            ...
            if number == 3:
                for role, names in [
                    ("supporting_evidence", ["wei"]),
                    ("not_relied_upon", ["biyikli", "oda"]),
                ]:
                    check(
                        role,
                        sorted(
                            re.sub(r"\s+et\s*al\.?$", "", c["display_name"].lower())
                            for c in result["citations"]
                            if c["citation_role"] == role
                        ),
                        names,
                    )
            ...
            def visit(value):
                if isinstance(value, dict):
                    if {"document_id", "start", "end", "text", "page_numbers"} <= value.keys():
                        ...
                        if value["text"] != doc["text"][value["start"] : value["end"]]:
                            failures.append(value["start"])
                        if value["page_numbers"] != [
                            p["number"]
                            for p in doc["pages"]
        ...
        (args.output / "golden-checks.json").write_text(
    ...
    raise SystemExit(
        0
        if all(
            r["http_status"] == 200 and all(c["pass"] for c in r["checks"].values())
            for r in reports
        )
        else 1
    )
